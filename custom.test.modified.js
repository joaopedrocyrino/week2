...
describe('Custom Tests', function () {
  ...
  it('[assignment] ii. deposit 0.1 ETH in L1 -> withdraw 0.08 ETH in L2 -> assert balances', async () => {
    ...
    const alicebEthAddress = '0xDeaD00000000000000000000000000000000BEEf'
    ...
    await transaction({
      ...
      recipient: alicebEthAddress,
    })

    // Assert recipient balance is right
    const aliceBalance = await token.balanceOf(alicebEthAddress)
    expect(aliceBalance).to.be.equal(aliceWithdrawAmount)

    // Assert omniBridge balance is right
    const omniBridgeBalance = await token.balanceOf(omniBridge.address)
    expect(omniBridgeBalance).to.be.equal(0)

    // Assert tornadoPool balance is right
    const tornadoBalance = await token.balanceOf(tornadoPool.address)
    expect(tornadoBalance).to.be.equal(aliceDepositAmount.sub(aliceWithdrawAmount))
  })
  ...
  it('[assignment] iii. see assignment doc for details', async () => {
    ...
    // Alice withdraws all remaining funds to L1
    ...
    const omniBridgeBalance = await token.balanceOf(omniBridge.address)
    expect(omniBridgeBalance).to.be.equal(aliceChangeUtxo.amount)

    // Bob withdraws his funds in L2
    const bobEthAddress = '0xDEAD00000000000000000000000000000000BEEE'
